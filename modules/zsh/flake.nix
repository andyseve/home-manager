{
  ...
  outputs =
    ...
    inputs.flake-utils.lib.eachDefaultSystem (system: {
      ...
      packages = foldl (
        plugacc: plugin: plugacc // { ${plugin} = pluginPackage system plugin; }
      ) { } plugins;
          ...
          config = mkIf true (mkMerge [
            (mkIf (plugins != [ ]) {
              home.file =
                ...
                foldl (
                  acc: plugin:
                  acc // { ".testdir/plugins/${plugin}".source = self.packages.${hostConfig.system}.${plugin}; }
                ) { } plugins;
            })
          ]);
        };

      nixosModules.default = { zshConfig, ... }: { };
